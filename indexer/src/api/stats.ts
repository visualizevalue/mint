...
import {
  getProtocolStats,
  getActivity,
  getArtifactCreations,
  getCollectionCreations,
  getTopSpenders,
  getTopEarners,
} from '../services/stats/protocol'
...
stats.get('/', async (c) => {
  ...
  const granularity =
    c.req.query('granularity') === 'weekly' ? ('weekly' as const) : ('daily' as const)

  const [stats, activity, artifactCreations, collectionCreations, topSpenders, topEarners] =
    await Promise.all([
