...
export async function getCollectionStats(db: any, { address }: { address: string }) {
  const addr = address.toLowerCase() as `0x${string}`

  const exists = await db
    .select({ address: collection.address })
    .from(collection)
  ...
  const [mintAgg, holderCount, artifactCount, topHolders, topCollectors, activity] =
    await Promise.all([
      db
        .select({
